...
Aim of spatial attention is to capture local features, this is done using dot product similarity un three feature maps \(Q,K,V\) by which we generate using a convolutional layer
** QKV Feature Maps
...
class SpatialAttention(nn.Module):
    def __init__(self , in_ch , heads):
        self.super.__init__()
        self.heads = heads
        self.feature_map = nn.Conv2d(in_ch , heads * 3 , kernel_size=1)
        return None
    ...
    def forward(self , x:Tensor):
        B , C , H , W = x.shape
        QKV:tuple[Tensor,Tensor,Tensor]  = self.feature_map(x).view(B,self.heads ,-1).chunk(3 , dim=1) # type: tuple[Tensor, Tensor, Tensor]
        Q,K,V = QKV
