...
class NoiseSchedule:
    ...
    def __init__(self,dev : pt.device ,  eps = 0.008 , T=1000 , linear=True):
        ...
        beta= pt.clip(beta, 0.0001, 0.999)
...
def sample_diffusion(T:int,model: nn.Module , shape:tuple , dev:pt.device , N , writer=None):
   eta = 0.9
   x_t = pt.randn(shape , device=dev)
   for t in reversed(range(0,T)):
      
      with pt.no_grad():
         predicted_noise = model(x_t,N.time[None,t])
      #sigma = pt.sqrt(
      #   N.beta[t] * (1 - N.alpha_bar[t-1]) / (1 - N.alpha_bar[t])
      #)
      weighted_noise = pt.sqrt(1-N.alpha_bar[t]).view(-1,1,1,1) * predicted_noise
      x_0 = (1/pt.sqrt(N.alpha_bar[t]).view(-1,1,1,1)) * (x_t - weighted_noise)
      if t>0:
          sigma_sqr = eta * (1. - N.alpha_bar[t-1])/(1.- N.alpha_bar[t])* (1. - N.alpha_bar[t]/N.alpha_bar[t-1])
          correction = pt.sqrt(1-N.alpha_bar[t-1] - sigma_sqr)
          noise = pt.randn(shape , device=dev)
          x_t = pt.sqrt(N.alpha_bar[t-1]).view(-1,1,1,1) * x_0 + pt.sqrt(sigma_sqr) * noise
      #x_t = x_t.clamp(-1,1)

      if writer is not None and t%(T//10)==0:
          log_samples(writer, x_0, step=T-t)
